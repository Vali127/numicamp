...
const Feeds = lazy(() => import("../../section/feed/feeds.jsx").then(m => ({ default: m.Feeds })))
const Notification = lazy(() => import("../../section/notification/notification.jsx").then(m => ({ default: m.Notification })))
const Resources = lazy(() => import("../../section/ressources/resources.jsx").then(m => ({ default: m.Resources })))
const Schools = lazy(() => import("../../section/school/schools.jsx").then(m => ({ default: m.Schools })))
...
const HomeContents = () => {
    const { logout, setLogout, userInfo, postModalVisibility, setPostModalVisibility, searchContent, setSearchContent, searched, setSearched } = PersonUiVm()
    ...
    useEffect(() => {
        if (section !== 'search') { setSearchContent(""); setSearched(false) }
    }, [section])
    useEffect(() => {
        if (userInfo.sexe) setUserType("personal")
    }, [userInfo])

    return (
        <div className="w-full flex relative">
            {postModalVisibility && (
                <PostModal profil={userInfo.photo_profil} name={userInfo.nom_personne} firstname={userInfo.prenom_personne} username={userInfo.nom_profil} domains={userInfo.domains} setModalVisibility={setPostModalVisibility} />
            )}
            ...
            <RightSidebar searchContent={searchContent} setSearchContent={setSearchContent} searched={searched} setSearched={setSearched} setSection={setSection} />
            <button onClick={() => setPostModalVisibility(true)} className="md:hidden fixed bottom-6 right-6 bg-indigo-500 text-white p-4 rounded-full shadow-lg hover:bg-indigo-600 transition-colors z-30" aria-label="Create post">
...
const LeftSidebar = ({ userInfo, section, setSection, logout, setLogout, setPostModalVisibility }) => (
    <div className="w-[20vw] hidden md:flex h-screen sticky top-0 p-2 flex-col justify-between">
        <div className="flex flex-col gap-10">
            <UserCard setSection={setSection} profile={userInfo.photo_profil} name={userInfo.nom_personne} firstname={userInfo.prenom_personne} user={userInfo.nom_profil} postModalIsVisible={setPostModalVisibility} />
            ...
            <div className="p-2 bg-neutral-50 sticky top-0">
                <form onSubmit={handleSearchSubmit}>
                    <div className="relative">
                        <input value={searchContent} onChange={(e) => setSearchContent(e.target.value)} type="text" className="text_input input__shadow rounded-2xl w-full pr-8" placeholder="rechercher ici ..." />
                        <button type="submit"><Search className="text-gray-700 scale-90 absolute right-2 top-1.5" /></button>
                    </div>
                </form>
