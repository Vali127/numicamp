...
const UniqueResource = ({illustration, title, description, date, favicon, link, domainName, source}) => {
    ...
    return (
        <div className='relative bg-white p-3 sm:p-4 rounded-lg flex flex-col gap-2 sm:gap-3 mb-1  text-left bg-gradient-to-b from-gray-50/60 to-white shadow-lg border border-gray-200'>
            {/* Favicon en haut à gauche */}
            <div className='w-5 h-5 sm:w-6 sm:h-6 rounded-full flex items-center justify-center overflow-hidden absolute top-2 left-2 bg-white border border-neutral-200'>
                <img
            ...
            {/* Contenu principal */}
            <div className='flex flex-col sm:flex-row gap-3 sm:gap-4 pl-7 sm:pl-8'>
                <div className='flex-1 flex flex-col gap-2'>
                    {/* Titre */}
                    <div className='font-bold text-base sm:text-lg md:text-xl leading-tight break-words'>{title}</div>
                ...
                {/* Image avec gradient */}
                <div className='relative bg-neutral-300 h-40 sm:h-32 w-full sm:w-32 rounded-md flex items-center justify-center overflow-hidden flex-shrink-0'>
                    <img
                        src={imageToDisplay}
            ...
            <div className='flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pt-2 border-t border-neutral-200'>
                <div className='flex flex-col gap-1 text-xs sm:text-sm min-w-0 flex-1'>
        ...
        <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className='bg-slate-600/0 !text-slate-600 px-2 py-1 rounded-2xl flex items-center gap-1 sm:gap-2 transition-colors duration-200 text-xs sm:text-sm font-medium whitespace-nowrap flex-shrink-0'>
